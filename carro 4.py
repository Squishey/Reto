...
class Carro:
    ...
    def _debe_frenar(self, semaforo, otros_carros):
        ...
        for otro in otros_carros:
            ...
            # Evita que carros de otro carril te hagan frenar "fantasmamente"
            if getattr(self, 'mi_semaforo', None) is not getattr(otro, 'mi_semaforo', None):
                continue
                
            if otro.posicion[0] > self.posicion[0]:
                if self._distancia(self.posicion, otro.posicion) < DISTANCIA_COLISION:
                    return True
    ...
    def actualizar(self, semaforo, otros_carros):
        ...
        if self.en_curva and self.curva_pts is not None:
            # Avanzar el parámetro t para que la velocidad visual sea constante
            p0, p1, p2 = self.curva_pts
            tangente = bezier_tangente(self.t, p0, p1, p2)
            magnitud_tangente = np.linalg.norm(tangente)
            
            if magnitud_tangente > 0:
                dt = self.velocidad / magnitud_tangente
            else:
                dt = 0.01
